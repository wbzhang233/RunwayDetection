...
int main(int argc, char** argv)
{
    ...
    while(framecount<290) {
        ...
        cout << filename << endl;
        // Loads an image
        Mat image = cv::imread(filename, IMREAD_REDUCED_GRAYSCALE_2);
        ...
        char string1[50];
        sprintf(string1,"lsd-result/%d.png",framecount);
        cv::imwrite(string1, line_image_lsd);
        // Show found lines with FLD
        Mat line_image_fld(image);
        fld->drawSegments(line_image_fld, lines_fld);
        imshow("FLD result", line_image_fld);
        char string2[50];
        sprintf(string2,"fld-result/%d.png",framecount);
        cv::imwrite(string2, line_image_fld);
        waitKey(10);
        ++framecount;
    }
    return 0;
